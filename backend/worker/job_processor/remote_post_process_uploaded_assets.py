    ...
    async def process(
        ...
        async with async_tempdir() as root_dir:
            try:
                ...
                # Kick off batched radar reverse geocoding api calls
                ...
                # Set up upload task
                upload_task = self.asset_manager.upload_files_batched(
                    [
                ...
                # Set up Radar reverse geocoding task
                radar_result_or_exc, upload_result_or_exc = await asyncio.gather(
                    geocode_task,
                    upload_task,
                ...
                if isinstance(radar_result_or_exc, BaseException):
                    logging.error(f"[geocode] Radar task failed: {radar_result_or_exc}")
                    radar_results = {}
                else:
